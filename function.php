...
/*function checkAuthenticate()
{
    if (isset($_SERVER['PHP_AUTH_USER'])  && isset($_SERVER['PHP_AUTH_PW'])) {
        if ($_SERVER['PHP_AUTH_USER'] != "diana" ||  $_SERVER['PHP_AUTH_PW'] != "diana12345") {
...
function sendEmail($to , $title , $body){
   ...
   // echo "Success" ; 
    ...
    function result($count,$success=null,$failure=null){
        if ($count > 0) {
            echo json_encode(array("status"=>"success","message"=>$success));
        } else {
            echo json_encode(array("status"=>"fail","message"=>$failure));
